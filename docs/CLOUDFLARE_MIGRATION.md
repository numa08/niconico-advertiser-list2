...
フロントエンドが満たすべき要件の詳細は [FRONTEND_REQUIREMENTS.md](./FRONTEND_REQUIREMENTS.md) に EARS 記法で整理してある。以下の12〜16はその要約であり、脱kobweb（React再実装）時の受け入れ基準は同ドキュメントの `FE-nnn` を正とする。
...
16. ページが読み込まれたとき、GA4測定タグが送信されなければならない。ただし現行はSPAのルート遷移で `page_view` が送信されない不具合があり、移行後はルート遷移のたびに送信する（FRONTEND_REQUIREMENTS.md の FE-151 が正）。
